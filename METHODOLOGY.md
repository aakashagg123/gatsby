...
build track: the lesson shape, the artifacts, and the way phases stack. It complements
the conceptual modules in [`content/`](./content/) with code you write yourself.
...
- The existing [`content/`](./content/) modules are the **Concept** reading each build
  lesson links to — we build *on top* of them, not instead of them.
