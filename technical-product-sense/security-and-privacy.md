...
minimize**. Ask "who can see this?" about every new surface. Assume every input — form
field, file upload, API call, prompt — is hostile until validated. Privacy is the
...
> Training and feedback data are a new privacy commitment. A helpful agent with broad
> permissions is a phishable employee that works at machine speed.
>
...
> retrieval design. "Can we learn from user data?" becomes a question you answer with
> counsel *before* the flywheel depends on it.
>
...
- **All input is hostile.** Form fields, file names, imported CSVs, webhook payloads,
  ...
  relevant (prompt injection). The defense always takes the same shape: validate,
...
- **Think in blast radius.** Perfect prevention doesn't exist, so bound the damage.
  Use least-privilege credentials (the reporting service can *read* orders, not delete
  them), keep secrets in a vault rather than code, enforce tenant isolation at the
  data layer, and keep audit logs so you can answer *what happened?* A useful review
  question for any component: "if this one thing were compromised tonight, what does
  the attacker hold tomorrow?"
...
- **Authorization by obscurity** — "Nobody will guess that URL/ID." They will, with a
  for-loop.
- **The checklist at the end** — Security review gets scheduled after the design is
  frozen, when every finding is expensive.
- **Data hoarding** — Everything gets collected "in case it's useful," and then a
  breach (or a deletion request) reveals exactly how much you held.
- **One credential to rule them all** — A service account with god-mode scopes is
  shared across features. One leak, total loss.
- **Privacy policy fiction** — The policy promises deletion and purpose-limits the
  systems don't implement. The gap gets discovered by a regulator or a journalist.
