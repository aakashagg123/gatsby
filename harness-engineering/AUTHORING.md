...
Every lesson in *Harness Engineering from Scratch* has the **same shape**. Readers never
re-learn the navigation, and the build script can parse every lesson the same way.
...
## Use It
Do the same task with the real SDK or framework — the Anthropic SDK, the MCP SDK, and
so on. The abstraction stays transparent, because the reader already built the toy
version.
...
Each question is multiple choice: 3–4 options, exactly one correct. Put the answer in a
collapsed block so readers self-test first:
...
Phase-level quizzes come from the [`check-understanding`](../.claude/skills/check-understanding/SKILL.md)
skill. It reads every `docs/en.md` in a phase and produces 8 questions — 4
conceptual and 4 practical. Placement is handled by
